...
export async function updatePerformanceSummary(studentId) {
  ...
  // Calculate Performance Index: 70% Academic, 20% Attendance, 10% Behavior
  const academicComponent = (overallPercentage / 100) * 70;
  const attendanceComponent = (attendance.percentage / 100) * 20;
  
  let behaviorScore = 0;
  if (behavior) {
     const avgBehavior = (behavior.communication_score + behavior.leadership_score + behavior.teamwork_score + behavior.participation_score + behavior.creativity_score + behavior.confidence_score + behavior.discipline_score) / 7;
     // behavior is out of 5. Convert to percentage.
     behaviorScore = (avgBehavior / 5) * 10; 
  }
  
  const performanceIndex = academicComponent + attendanceComponent + behaviorScore;
