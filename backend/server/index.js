...
app.get("/api/all", async (req, res) => {
  ...
  try {
    const [
      ...
      runQuery(db, "SELECT id, topic_id, 'ppt' AS type, CONVERT(COALESCE(title,'PPT') USING utf8mb4) AS title, CONVERT(ppt_url USING utf8mb4) AS url, is_mandatory FROM topic_ppt_materials UNION ALL SELECT id, topic_id, 'pdf' AS type, CONVERT(COALESCE(title,'PDF') USING utf8mb4) AS title, CONVERT(pdf_url USING utf8mb4) AS url, is_mandatory FROM topic_pdf_materials UNION ALL SELECT id, topic_id, 'youtube' AS type, CONVERT(COALESCE(title,'Video') USING utf8mb4) AS title, CONVERT(url USING utf8mb4) AS url, is_mandatory FROM topic_youtube_links UNION ALL SELECT id, topic_id, 'activity' AS type, CONVERT(COALESCE(title,'Activity') USING utf8mb4) AS title, CONVERT(COALESCE(activity_url, description) USING utf8mb4) AS url, is_mandatory FROM topic_activity_materials"),
